...
import { can, VALID_ROLES, rankOf, canGrantRole } from './rbac';
import { viewerLeadsEvent } from './event-view';
import { resolveLiveRole } from './auth';
import type { EventPayload, EventDoc, CasePayload, CaseDoc, CaseSize, UserDoc, Role, CaseSignoff, EventAuditEntry } from './types';
import type { InventoryDoc, InventoryPayload, DistributionRow, ItemUnit, ItemFlag, SkuOption, KitRequirement } from './inventory-shape';
import { addFlag as buildAddFlag } from './inventory-shape';
import { buildManifestSnapshot, buildCheckinSweep, type SnapshotCaseLite } from './signoff-view';
import type { TagDoc, TagPayload } from './data';

// lib/write.ts — the single server-side write path for the Next.js stack.
//
...
export async function retireOrDeleteCase({
  ...
  const { classifyCaseDelete } = await import('./case-view');
...
export async function cycleItemStateInCase({
  ...
  const { itemStateInCase } = await import('./inventory-shape');
...
export interface CommitReadyResult {
  ...
  snapshot: import('./types').ManifestSnapshot | null;
}
export async function commitEventReady({ eventId, shipping, actor }: CommitReadyArgs): Promise<CommitReadyResult> {
  ...
  const { eventCanCommitReady } = await import('./signoff-view');
...
export async function commitEventClosed({ eventId, actor }: CommitClosedArgs): Promise<{ ok: boolean }> {
  ...
  const inv = (await db.collection<InventoryDoc>('inventory').find(NOT_DELETED).toArray()).map((d) => d.payload);
  const { eventCanCommitClosed } = await import('./signoff-view');
