...
// ── Shipping tab ─────────────────────────────────────────────────────────────────────────────
function ShippingPanel({ p, eventId, canEdit }: { p: EventPayload; eventId: string; canEdit: boolean }) {
  const out = p.outbound ?? {};
  const ret = p.return ?? {};
  const bols = Array.isArray(p.bols) ? p.bols : [];
  return (
    <>
      <FieldGroup title="Outbound shipping">
        ...
        <BolList eventId={eventId} direction="outbound" bols={bols} pallets={p.pallets ?? []} canEdit={canEdit} />
      </FieldGroup>
      ...
      <FieldGroup title="Return shipping">
        ...
        <BolList eventId={eventId} direction="return" bols={bols} pallets={p.pallets ?? []} canEdit={canEdit} />
      </FieldGroup>
    </>
  );
}
...
function BolList({ eventId, direction, bols, pallets, canEdit }: { eventId: string; direction: 'outbound' | 'return'; bols: EventBol[]; pallets: EventPallet[]; canEdit: boolean }) {
  const router = useRouter();
    ...
    <div className="flex flex-col gap-4 px-4 py-4 sm:px-6 sm:py-6">
      ...
      <div>
        ...
        <div role="tabpanel" hidden={active !== 'shipping'}>
          <ShippingPanel p={payload} eventId={eventId} canEdit={canEdit} />
        </div>
