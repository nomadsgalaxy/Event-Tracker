...
// editable: url/events/method/secret/description/active). DELETE — remove it. Admin-owned write
// key for both (see ../route.ts).
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withKey(req, async (vk) => {
    if (rankOf(vk.role) < rankOf('admin')) return apiErr(403, 'Webhook management requires an admin-owned key.');
    requireScope(vk, 'db.write.app');
    const res = await removeWebhookSubscription(id);
